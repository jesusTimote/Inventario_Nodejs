...
$(document).on('nifty.ready', function () {
    ...
    //filterSearch
    // -----------------------------------------------------------------
    var filterSearch = $('#demo-foo-filterSearch');
    filterSearch.footable().on('footable_filtering', function (e) {
        var selected = $('#demo-foo-filter').find(':selected').val();
        e.filter += (e.filter && e.filter.length > 0) ? ' ' + selected : selected;
        e.clear = !e.filter;
    });
    // Filter status
    $('#demo-foo-filter').change(function (e) {
        e.preventDefault();
        var pageSize = $(this).val();
        filterSearch.data('page-size', pageSize);
        filterSearch.trigger('footable_initialized');
    ...
    $('#demo-foo-search').on('input', function (e) {
        e.preventDefault();
        filterSearch.trigger('footable_filter', { filter: $(this).val() });
    });
